...
import (
	...
	authutil "github.com/hoangndst/cloud-observability-backend/util/auth"
)

const (
	headerName = "X-CloudObser-Key"
)
	...
	return func(ctx *gin.Context) {
		...
		if user != nil || token != "" {
			...
			if err != nil {
				...
				authutil.RegisterAuthentication(ctx, user, userID, token)
				ctx.Next()
				return
...
func (a *Auth) Optional() gin.HandlerFunc {
	return func(ctx *gin.Context) {
		...
		user, err := a.userFromBasicAuth(ctx)
		if err != nil {
			authutil.RegisterAuthentication(ctx, nil, 0, "")
			ctx.Next()
			return
		}

		if user != nil {
			authutil.RegisterAuthentication(ctx, user, user.ID, token)
			ctx.Next()
			return
		} else if token != "" {
			if tokenClient, err := a.DB.GetClientByToken(token); err == nil && tokenClient != nil {
				authutil.RegisterAuthentication(ctx, user, tokenClient.UserID, token)
				ctx.Next()
				return
			}
		}
